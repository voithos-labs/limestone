<script lang="ts">
    ...
    let {open = $bindable(false), onPicked}: {
        open: boolean;
    ...
    async function openFile() {
        const sel = await openDialog({multiple: false, filters: [{name: 'Images', extensions: IMAGE_EXTS}]});
...
{#if open}
    <div class="overlay" onclick={() => open = false} onkeydown={onKey} role="presentation">
        <div class="dialog" class:drag={dragOver} onclick={(e) => e.stopPropagation()} onkeydown={onKey} role="dialog" tabindex="-1">
