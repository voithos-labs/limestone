<script lang="ts">
	...
	import {
		...
		rawArrayValue as rawArray,
		tagClass as tagClassOf,
		valueFor as valueOf,
		titleFor as titleOf,
		folderDir,
		fileName,
		sourceName as sourceNameOf,
		isEditable,
		isMetaField,
		fieldLabel,
		withStatefulValue
	...
	function onGroupPointerDown(e: PointerEvent, label: string) {
		...
		function move(ev: PointerEvent) {
			...
			const headers = [
				...(tableEl?.querySelectorAll('tr.group-header[data-group-key]') ?? [])
			] as HTMLElement[];
		...
		function up() {
			...
			if (started && dragGroupKey && dropTargetKey)
				commitGroupReorder(dragGroupKey, dropTargetKey, dropAfter);
			dragGroupKey = null;
			dropTargetKey = null;
		}
	...
	const rowMenuItems = [{ value: 'delete', label: 'Delete', icon: Trash2 }];
	...
	async function commitNew() {
		...
		try {
			...
			queueMicrotask(() =>
				startNew(groupKey !== null ? { groupValue: gv, groupKey } : { floatTop: wasFloat })
			);
		} catch (e) {
			error = String(e);
		} finally {
...
<LeanScroll {flow}>
	...
	>
		{#snippet draftRow_(sticky = false)}
			<tr
				class="new-row creating"
				class:float-top={sticky}
				bind:this={newRowEl}
				onkeydown={onDraftRowKey}
							...
							{#if !hasFolderColumn}
								...
								>
									...
									<span class="nr-float-label">
										{needsFolderChoice
											? 'Choose…'
											: folderDirLabel.split('/').pop() || createSourceName}
									</span>
								</button>
							{/if}
							<button
			...
			{#each renderItems as item, ri (item.kind === 'row' ? item.row.id : item.kind === 'header' ? 'h:' + item.label : 'f:' + item.key)}
				{#if item.kind === 'header'}
					<tr
						...
						class:drop-before-group={dragGroupKey !== null &&
							dragGroupKey !== item.label &&
							dropTargetKey === item.label &&
							!dropAfter}
				...
				{:else if item.kind === 'footer'}
					...
					{:else}
						<tr
							class="new-row group-add"
							class:drop-after-group={dragGroupKey !== null &&
								dragGroupKey !== item.key &&
								dropTargetKey === item.key &&
								dropAfter}
						>
...
{#if editingField && editingRow}
	{#if editingField.type === 'text'}
		<CellTextEditor
			...
			onCommit={(v) => {
				if (editingField && editingRow) writeCell(editingField, editingRow, v);
			}}
		/>
	{:else}
		<CellEditor
			...
			onChange={(v, dir) => {
				if (editingField && editingRow) writeCell(editingField, editingRow, v, dir);
			}}
			onRenameOption={(oldV, newV) => {
				if (editingField) renameOption(editingField, oldV, newV);
			}}
		/>
	{/if}
{/if}
