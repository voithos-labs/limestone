<script lang="ts">
        ...
        try {
            ...
            if (q) {
                ...
                if (ids.length === 0) {
                    ...
                    const members = (await view.getMembers({face, scope, ids_in: ids, limit: ids.length})) as Row[];
    ...
    async function fetchRowTags(list: Row[]): Promise<Record<string, { id: string; slug: string }[]>> {
