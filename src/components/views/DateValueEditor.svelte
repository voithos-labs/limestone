<script lang="ts">
	...
	const displayHour = $derived(meridiem === '24' ? pad(hh) : pad(hh % 12 === 0 ? 12 : hh % 12));
	...
	function parse(
		v: unknown
	): { y: number; m: number; d: number; hh: string; mm: string; time: boolean } | null {
	...
	function parseFlexible(
		raw: string
	): { y: number; m: number; d: number; hh: number | null; mm: number } | null {
	...
	function prefers24h(): boolean {
		try {
			return !Intl.DateTimeFormat([], { hour: 'numeric' }).resolvedOptions().hour12;
		} catch {
			return false;
		}
	}
