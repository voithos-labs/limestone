<script lang="ts">
    ...
    let {open = $bindable(false), anchor, onPick}: {
        open: boolean;
...
{#if open}
    <div class="ep" bind:this={popEl} style:top="{pos.top}px" style:left="{pos.left}px" role="menu" tabindex="-1">
        <div class="ep-search">
                ...
                {#if results.length}
                    <div class="ep-grid">
                        {#each results as e (e.slug)}
                            <button class="ep-cell" type="button" title={e.name}
