<script lang="ts">
    ...
    $effect(() => {
        if (open && !wasOpen) {
            ...
            if (mode === 'edit' && source) {
                ...
                getSourceConfig(source.id).then(c => {
    ...
    async function submit() {
        error = '';
        const config = {note_location: noteLocation.trim(), asset_location: assetLocation.trim() || 'assets'};
        busy = true;
        try {
            if (mode === 'create') {
                if (!folderPath) {
...
{#if open}
    <div class="overlay" onclick={() => open = false} onkeydown={onKey} role="presentation">
        <div class="dialog" onclick={(e) => e.stopPropagation()} onkeydown={onKey} role="dialog" tabindex="-1">
            <h3 class="title">{mode === 'create' ? 'Add source' : sourceName(source ?? {path: '', title: ''})}</h3>
            ...
            <label class="field">
                ...
                {#if mode === 'create'}
                    <button class="folder-pick" type="button" onclick={chooseFolder}>
                        <Folder size={14}/>
                        <span class="folder-text" class:placeholder={!folderPath}>{folderPath || 'Choose folder…'}</span>
                    </button>
                {:else}
            ...
            <label class="field">
                ...
                <input class="input" type="text" bind:value={noteLocation} placeholder="(source root)" spellcheck="false"/>
            </label>
