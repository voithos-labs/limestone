...
import { buildRecoveryRegionProofs, parseMarkdownProofOfConcept, serializeProofOfConceptDocument, } from "../core/index.js";
import { PARSER_VALIDATION_CASE_GROUPS, PRESERVE_ONLY_AND_FALLBACK_CASES, } from "./fixtures/parser-validation-fixtures.js";
...
function assertParserFixture(fixture) {
    ...
    assert.equal(serializeProofOfConceptDocument(document), fixture.source, `Expected exact round-trip for fixture ${fixture.name}.`);
    assert.deepEqual(document.blocks.map((block) => block.kind), fixture.expectedBlockKinds, `Unexpected block kinds for fixture ${fixture.name}.`);
    if (fixture.expectedInlineTokens) {
        for (const expectation of fixture.expectedInlineTokens) {
            assert.deepEqual(document.blocks[expectation.blockIndex]?.inlineTokens.map((token) => token.kind) ?? [], expectation.kinds, `Unexpected inline token kinds for fixture ${fixture.name} block ${expectation.blockIndex}.`);
        }
    }
    if (fixture.expectedBlockMetadata) {
        for (const expectation of fixture.expectedBlockMetadata) {
            ...
            for (const [key, value] of Object.entries(expectation.metadata)) {
                assert.equal(actualMetadata?.[key], value, `Unexpected metadata key ${key} for fixture ${fixture.name} block ${expectation.blockIndex}.`);
            }
        }
    }
    if (fixture.expectedFenceClosed !== undefined) {
        const fencedBlock = document.blocks.find((block) => block.kind === "fencedCode");
        assert.equal(fencedBlock?.metadata.closed, fixture.expectedFenceClosed, `Unexpected fence closure state for fixture ${fixture.name}.`);
    }
    if (fixture.expectedRecoveryReasons) {
        assert.deepEqual(buildRecoveryRegionProofs(fixture.source).map((proof) => proof.reason), fixture.expectedRecoveryReasons, `Unexpected recovery reasons for fixture ${fixture.name}.`);
    }
}
