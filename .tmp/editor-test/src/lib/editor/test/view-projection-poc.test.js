...
import { buildViewProjectionProofOfConcept, findFirstViewProjectionBlockByRole, } from "../core/index.js";
...
test("View projection PoC produces the expected flat block stream", () => {
    ...
    assert.deepEqual(projection.blocks.map((block) => block.role), ["heading", "paragraph", "listItem", "listItem", "blockquote", "fencedCode", "thematicBreak"]);
});
test("View projection PoC carries container context and block metadata for structural blocks", () => {
    ...
    const taskListItem = projection.blocks.find((block) => block.role === "listItem" && block.metadata.taskItem === true);
...
test("View projection PoC flattens inline marks and preserves image atoms", () => {
    ...
    const imageListItem = projection.blocks.find((block) => block.role === "listItem" && block.runs.some((run) => run.atomic?.kind === "image"));
    assert.equal(paragraph !== undefined, true);
    assert.equal(paragraph?.runs.some((run) => run.marks.includes("strong")), true);
    assert.equal(paragraph?.runs.some((run) => run.marks.includes("strikethrough")), true);
    assert.equal(paragraph?.runs.some((run) => run.marks.includes("inlineCode")), true);
    assert.equal(paragraph?.runs.some((run) => run.marks.includes("autolink")), true);
    const imageRun = imageListItem?.runs.find((run) => run.atomic?.kind === "image");
...
test("View projection PoC leaves void blocks raw and fenced code plain-text editable", () => {
    ...
    assert.equal(codeBlock?.runs.some((run) => run.text.includes('console.log("hello");')), true);
    assert.equal(codeBlock?.runs.some((run) => run.marks.length > 0), false);
