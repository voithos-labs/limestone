...
import { applyInputHarnessBeforeInputIntent, buildInputHarnessProofOfConcept, createCollapsedInputHarnessTextSelection, createInputHarnessNodeSelection, createInputHarnessTextSelection, findFirstInputHarnessBlockByRole, mapInputHarnessLogicalSelectionToDomSelection, pasteIntoInputHarnessSelection, reconcileInputHarnessDomSelection, repairInputHarnessDomSelection, serializeInputHarnessSelection, startInputHarnessComposition, updateInputHarnessComposition, commitInputHarnessComposition, } from "../core/index.js";
...
test("Phase 0 input harness PoC preserves image atoms as node selections", () => {
    const document = buildInputHarnessProofOfConcept(MIXED_GFM_FIXTURE);
    const imageBlock = document.blocks.find((block) => block.parts.some((part) => part.kind === "atom"));
    assert.equal(imageBlock !== undefined, true);
    const imageAtom = imageBlock.parts.find((part) => part.kind === "atom");
