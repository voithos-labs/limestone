...
export function buildCstOwnershipProofOfConcept(source) {
...
export function validateCstOwnershipProof(proof) {
    const issues = [];
...
export function buildDelimiterAccountingProof(source) {
...
export function buildRecoveryRegionProofs(source) {
    const proofs = [];
...
function createBlockOwnership(nodeId, block, source) {
    switch (block.kind) {
        case "heading":
...
function createWholeTextOwnership(nodeId, block, source, flags) {
    return {
        tokens: [],
...
function createWholeTokenOwnership(nodeId, block, source, tokenKind) {
...
function createPrefixTokenOwnership(nodeId, block, source, pattern, tokenKind) {
    ...
    const segments = [createSegment("token", nodeId, tokenKind, token.range.start, token.range.end, source)];
...
function createBlockquoteOwnership(nodeId, block, source) {
    const tokens = [];
    const textRanges = [];
    const segments = [];
...
function createFencedCodeOwnership(nodeId, block, source) {
...
function splitRawLines(source, baseOffset) {
    const lines = [];
    let start = 0;
    for (let index = 0; index < source.length; index += 1) {
        if (source[index] === "\n") {
...
function findPartialLinkRange(source) {
...
function isCoveredByRanges(index, ranges) {
...
function createToken(kind, start, end, source) {
    return {
        kind,
...
function createSegment(kind, ownerId, label, start, end, source) {
    return {
        kind,
        ownerId,
        label,
