...
export function findFirstInputHarnessBlockByRole(document, role) {
    return document.blocks.find((block) => block.role === role);
}
export function createCollapsedInputHarnessTextSelection(document, blockKey, textOffset) {
    return createInputHarnessTextSelection(document, blockKey, textOffset, textOffset);
}
export function createInputHarnessTextSelection(document, blockKey, anchorTextOffset, focusTextOffset) {
...
export function createInputHarnessNodeSelection(document, atomKey) {
...
export function mapInputHarnessLogicalSelectionToDomSelection(document, selection) {
    return {
        anchor: mapLogicalPositionToDomPoint(document, selection.anchor),
        focus: mapLogicalPositionToDomPoint(document, selection.focus),
    };
}
export function reconcileInputHarnessDomSelection(document, domSelection) {
...
export function updateInputHarnessComposition(session, previewText) {
    return {
        ...session,
        previewText,
    };
}
export function commitInputHarnessComposition(document, session, finalText) {
...
function replaceInputHarnessSelection(document, selection, insertedText) {
    ...
    const nextSelection = createSelection(resolveSourceOffsetToLogicalPosition(nextDocument, range.start + insertedText.length), resolveSourceOffsetToLogicalPosition(nextDocument, range.start + insertedText.length));
    return {
        source,
...
function buildEditableSlices(block) {
    switch (block.kind) {
        case "paragraph":
...
function createTrimmedBlockSlice(block) {
...
function createHeadingSlice(block) {
    const trimmed = trimTrailingLineEnding(block.source);
...
function createListItemSlice(block) {
    const trimmed = trimTrailingLineEnding(block.source);
...
function createBlockquoteSlices(block) {
...
function createFencedCodeBodySlice(block) {
    const lines = splitLinesWithOffsets(block.source, block.range.start);
...
function buildInlineParts(blockKey, block, slices, editable) {
...
function createTextPart(blockKey, index, sourceStart, sourceEnd, textStart, text) {
    return {
        kind: "text",
...
function createAtomPart(blockKey, index, token, textStart) {
    return {
        kind: "atom",
...
function mapLogicalPositionToDomPoint(document, position) {
...
function reconcileDomPointToLogicalPosition(document, domPoint) {
...
function resolveTextOffsetToLogicalPosition(block, requestedTextOffset) {
...
function resolveSourceOffsetToLogicalPosition(document, requestedSourceOffset) {
...
function resolveSourceOffsetWithinBlock(block, requestedSourceOffset) {
...
function resolveBoundaryBeforePart(block, part) {
    if (part.kind === "atom") {
        return {
            kind: "atom",
            blockKey: block.key,
...
function resolveBoundaryAfterPart(block, part) {
    if (part.kind === "atom") {
        return {
            kind: "atom",
            blockKey: block.key,
...
function getTextPartForTextOffset(block, textOffset) {
    const textParts = block.parts.filter((part) => part.kind === "text");
...
function getBlockByKey(document, blockKey) {
...
function getPartByKey(block, partKey) {
...
function getAtomByKey(document, atomKey) {
    for (const block of document.blocks) {
...
function compareLogicalPositions(left, right) {
...
function toOrderedSourceRange(selection) {
    return compareLogicalPositions(selection.anchor, selection.focus) <= 0
...
function splitLinesWithOffsets(source, baseOffset) {
    if (source.length === 0) {
        return [];
    }
    const lines = [];
...
function createRawLine(raw, start) {
...
function trimTrailingLineEnding(value) {
