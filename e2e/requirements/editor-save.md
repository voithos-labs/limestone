...
- Closing the window writes an edit that no save window has reached yet. The window is destroyed
  only after the flush, so a document is never left on disk older than the one on screen. A flush
  writes the editor's live source, not the last edit event — those lag a typing burst.
...
- Deleting a document does not write it back. Deleting closes its tab, and that teardown flushes
  like any other — but the file is gone, and a flush reading the live source would recreate it.

## Edge cases

- An empty document is typable on open too — there is no first block to click.
- A fast burst of keystrokes settles on a write carrying all of them, possibly after more than
  one: the editor commits the first character alone and the rest on a debounce longer than this
  adapter's, so a burst crosses two save windows. Each write is a truthful snapshot, so a scenario
  pins the settled content and that writes are nowhere near one per keystroke — never the count.
